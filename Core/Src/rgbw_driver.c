...
HAL_StatusTypeDef rgbw_driver_set_channel_brightness(rgbw_driver_channels_t channel, uint8_t brightness){
	...
	switch(channel){
		case RED:
			tmp_reg_addres = RGBW_CHIP_REG_CHANNEL_RED;
			break;
		case GREEN:
			tmp_reg_addres = RGBW_CHIP_REG_CHANNEL_GREEN;
			break;
		case BLUE:
			tmp_reg_addres = RGBW_CHIP_REG_CHANNEL_BLUE;
			break;
		case WHITE:
			tmp_reg_addres = RGBW_CHIP_REG_CHANNEL_WHITE;
			break;
		default:
			return HAL_ERROR;
	}
